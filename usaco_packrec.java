...
import java.util.*;
import java.io.*;
public class usaco_packrec {
	static int min = 1000000;
	static ArrayList<int[]> a = new ArrayList<int[]>();
	public static void main(String args[]) throws IOException{
    	Scanner in = new Scanner(new File("packrec.in"));
    	FileWriter fstream = new FileWriter("packrec.out");
		...
		for(int i=1;i<a.size();i++){
			if(a.get(i)[0]!=p){
				p = a.get(i)[0];
				out.write(String.valueOf(a.get(i)[0])+" "+String.valueOf(a.get(i)[1])+"\n");
			}
		}
        out.close();
	}
